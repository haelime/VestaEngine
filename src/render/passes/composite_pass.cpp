...
BloomPass::BloomPass(BloomPassStage stage)
    : _stage(stage)
{
}

void BloomPass::SetInput(GraphTextureHandle input)
{
    _input = input;
}

void BloomPass::SetSecondaryInput(GraphTextureHandle input)
{
    _secondaryInput = input;
}

void BloomPass::SetOutput(GraphTextureHandle output)
{
    _output = output;
}

void BloomPass::SetParameters(float threshold, float intensity)
{
    _threshold = std::clamp(threshold, 0.0f, 8.0f);
...
{
    switch (_stage) {
    case BloomPassStage::Extract:
        return "Bloom ExtractPass";
    case BloomPassStage::Downsample:
        return "Bloom DownsamplePass";
    case BloomPassStage::Upsample:
        return "Bloom UpsamplePass";
    }
    return "BloomPass";
}

void BloomPass::Initialize(RenderDevice& device)
{
...
void BloomPass::Setup(RenderGraphBuilder& builder)
{
    ...
    if (_secondaryInput) {
        builder.Read(_secondaryInput, ResourceUsage::StorageRead);
    }
    builder.Write(_output, ResourceUsage::ColorAttachmentWrite);
}

void BloomPass::Execute(const RenderGraphContext& context)
{
    if (_pipeline == VK_NULL_HANDLE || !_input || !_output) {
        return;
    }

    constexpr uint32_t kBloomExtractMode = 90u;
    constexpr uint32_t kBloomDownsampleMode = 91u;
    constexpr uint32_t kBloomUpsampleMode = 92u;
    const uint32_t shaderMode = _stage == BloomPassStage::Extract ? kBloomExtractMode
        : _stage == BloomPassStage::Downsample                    ? kBloomDownsampleMode
                                                                  : kBloomUpsampleMode;
    ...
    CompositePushConstants pushConstants{
        ...
        .imageIndices1 = glm::uvec4(shaderMode, 0u, 0u, kInvalidImageIndex),
        ...
        .gaussianDebug = glm::uvec4(kInvalidImageIndex, 0u, 0u, 0u),
        .params = glm::vec4(0.0f, 0.0f, 0.05f, 500.0f),
        .compareParams = glm::vec4(0.0f, 0.5f, 4.0f, 2.0f),
        .postParams = glm::vec4(1.0f, 1.0f, 0.0f, 0.0f),
        .bloomParams = glm::vec4(_threshold, _intensity, 1.0f, 0.0f),
        .ssaoParams = glm::vec4(1.0f, 0.75f, 1.35f, 0.0f),
        .motionBlurParams = glm::vec4(0.0f, 0.0f, 5.0f, 0.0f),
        .inverseViewProjection = glm::mat4(1.0f),
    };

    const ImageHandle inputHandle = context.GetTextureHandle(_input);
    pushConstants.imageIndices0.x = context.GetDevice().GetImageResource(inputHandle).bindless.storageImage;
    if (_secondaryInput) {
        const ImageHandle secondaryHandle = context.GetTextureHandle(_secondaryInput);
        pushConstants.imageIndices0.y = context.GetDevice().GetImageResource(secondaryHandle).bindless.storageImage;
    }
...
void BloomPass::Shutdown(RenderDevice& device)
{
    VkDevice vkDevice = device.GetDevice();
...
void CompositePass::SetBloomInput(GraphTextureHandle bloom)
{
    _bloom = bloom;
}
...
void CompositePass::Setup(RenderGraphBuilder& builder)
{
    ...
    if (_bloom) {
        builder.Read(_bloom, ResourceUsage::StorageRead);
    }
    builder.Write(_output, ResourceUsage::ColorAttachmentWrite);
}
...
void CompositePass::Execute(const RenderGraphContext& context)
{
    ...
    if (_bloom && _debugView != 21u) {
        const ImageHandle handle = context.GetTextureHandle(_bloom);
        pushConstants.imageIndices4.y = context.GetDevice().GetImageResource(handle).bindless.storageImage;
    }
